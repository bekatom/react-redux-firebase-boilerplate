...
const FireBaseTools = {
   ...
   */
    loginWithProvider: (p) => {
        ...
        return firebaseAuth.signInWithPopup(provider).then(result => firebaseAuth.currentUser).catch(error => ({
   ...
   * @returns {any|!firebase.Thenable.<*>|firebase.Thenable<any>}
   */
    registerUser: user => firebaseAuth.createUserWithEmailAndPassword(user.email, user.password).then(user => user).catch(error => ({
        errorCode: error.code,
        errorMessage: error.message,
    })),
   ...
   * @returns {any|!firebase.Thenable.<*>|firebase.Thenable<any>}
   */
    loginUser: user => firebaseAuth.signInWithEmailAndPassword(user.email, user.password).then(user => user).catch(error => ({
        errorCode: error.code,
        errorMessage: error.message,
    })),
